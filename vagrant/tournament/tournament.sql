...
create table players if not exists (player_id smallserial primary key, name text);

create table matches if not exists (match_id smallserial primary key, winner_id integer references players (player_id), loser_id integer references players (player_id));
